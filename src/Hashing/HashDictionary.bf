...
namespace RfgTools.Hashing
{
	...
	{
        private static append Dictionary<u32, StringView> _stringDictionary0;
        private static append Dictionary<u32, StringView> _stringDictionary1;
        private static append Dictionary<u32, StringView> _stringDictionary2;
        private static append Monitor _initLock;

        public static bool Initialized { get; private set; } = false;

        public static void Initialize()
        {
            ScopedLock!(_initLock);
            if (Initialized) //Check after lock in case another thread got to it first
                return;

            for (StringView str in _sourceStrings)
            {
                _stringDictionary0[Hash.HashVolitionCRC(str, 0)] = str;
                _stringDictionary1[Hash.HashVolitionCRCAlt(str, 0)] = str;
                _stringDictionary2[Hash.HashVolition(str)] = str;
            }
            Initialized = true;
        }

        public static Result<StringView> FindOriginString(u32 hash)
        {
            if (!Initialized)
                Initialize();

            if (_stringDictionary0.ContainsKey(hash))
                return _stringDictionary0[hash];
            if (_stringDictionary1.ContainsKey(hash))
                return _stringDictionary1[hash];
            if (_stringDictionary2.ContainsKey(hash))
	            return _stringDictionary2[hash];

            return .Err;
        }

        private static StringView[?] _sourceStrings =
	    .(
	        //District names
